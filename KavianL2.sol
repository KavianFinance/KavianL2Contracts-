...
// KavianL2 with Governance.
contract KavianL2 is BEP20 {
	...
	uint16 public maxTransferAmountRate = 800;
	...
	// Min amount to liquify. (default 25 KAVIANL2)
	...
	modifier antiWhale(
		...
		if (maxTransferAmount() > 0) {
			if (_excludedFromAntiWhale[sender] == false && _excludedFromAntiWhale[recipient] == false) {
				require(amount <= maxTransferAmount(), "KAVIANL2::antiWhale: Transfer amount exceeds the maxTransferAmount");
			}
		}
		_;
	}
	...
	/**
	 * @notice Constructs the KavianL2 contract.
	 */
	constructor() public BEP20("KAVIAN L2 Token", "KAVIANL2") {
	...
	/// @dev overrides transfer function to meet tokenomics of KAVIANL2
	function _transfer(
		address sender,
		address recipient,
		uint256 amount
		...
		if (recipient == BURN_ADDRESS || recipient == owner() || sender == owner() || transferTaxRate == 0) {
			...
			require(taxAmount == burnAmount + liquidityAmount, "KAVIANL2::transfer: Burn value invalid");

			// default 95% of transfer sent to recipient
			uint256 sendAmount = amount.sub(taxAmount);
			require(amount == sendAmount + taxAmount, "KAVIANL2::transfer: Tax value invalid");
	...
	function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
		require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "KAVIANL2::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
	...
	function updateBurnRate(uint16 _burnRate) public onlyOperator {
		require(_burnRate <= 100, "KAVIANL2::updateBurnRate: Burn rate must not exceed the maximum rate.");
	...
	function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
		require(_maxTransferAmountRate <= 10000, "KAVIANL2::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
		require(
			_maxTransferAmountRate >= maxTransferAmountRateMinValue,
			"KAVIANL2::updateMaxTransferAmountRate: Max transfer amount rate must be grater than min rate"
		);
	...
	function setSwapPair() public onlyOperator {
		...
		require(swapPair != address(0), "KAVIANL2::updateSwapRouter: Invalid pair address. Check if liquidity was added");
	}
	...
	function transferOperator(address newOperator) public onlyOperator {
		require(newOperator != address(0), "KAVIANL2::transferOperator: new operator is the zero address");
	...
	function delegateBySig(
		...
		require(signatory != address(0), "KAVIANL2::delegateBySig: invalid signature");
		require(nonce == nonces[signatory]++, "KAVIANL2::delegateBySig: invalid nonce");
		require(now <= expiry, "KAVIANL2::delegateBySig: signature expired");
	...
	function getPriorVotes(address account, uint256 blockNumber) external view returns (uint256) {
		require(blockNumber < block.number, "KAVIANL2::getPriorVotes: not yet determined");
	...
	function _delegate(address delegator, address delegatee) internal {
		...
		uint256 delegatorBalance = balanceOf(delegator); // balance of underlying KAVIANL2 (not scaled);
	...
	function _writeCheckpoint(
		...
		uint32 blockNumber = safe32(block.number, "KAVIANL2::_writeCheckpoint: block number exceeds 32 bits");
